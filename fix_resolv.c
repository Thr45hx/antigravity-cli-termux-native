 ...
 *
 * glibc's resolver opens /etc/resolv.conf for DNS, but Termux's /etc is a
 * read-only symlink to /system/etc, so that file cannot exist natively. Instead
 * of proot, this shim intercepts open()/fopen() of /etc/resolv.conf and
 * redirects it to Termux's copy ($PREFIX/etc/resolv.conf).
 *
 * Shared with the claude-code-termux-native project. For Antigravity (a Go
 * binary) the launcher sets GODEBUG=netdns=cgo so DNS goes through glibc's
 * resolver (whose fopen this shim catches) instead of Go's raw-syscall resolver.
 *
 * Redirect target overridable at runtime via env CLAUDE_RESOLV_REDIRECT.
 */
